...
def run_analysis(link, settings, collector):
    ...
    try:
        ...
        cmd_info = f'{y_p} --user-agent "{UA}" --extractor-args "youtube:player_client=android" --skip-download --write-info-json -o "{TEMP_DIR}/{sid}_full" "{link}"'
        ...
        # Try subtitles
        cmd_subs = f'{y_p} --user-agent "{UA}" --extractor-args "youtube:player_client=android" --skip-download --write-auto-subs --sub-langs "id,en" --convert-subs srt -o "{TEMP_DIR}/{sid}_full" "{link}"'
        try:
