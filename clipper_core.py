"""
clipper_core.py — Core logic for YT Short Clipper.
...
def process_single_video(link, start_sec, end_sec, title, lang, model_size, log_func, progress_func, opts=None):
    ...
    try:
        ...
        log_func(f"[{safe_id}] 🎬 Proses frame...")
        cap = cv2.VideoCapture(str(trimmed))
        fps = cap.get(cv2.CAP_PROP_FPS) or 30.0
        frame_w = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
        frame_h = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
        total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
        res_parts = export_resolution.split("x")
        target_w = int(res_parts[0])
        target_h = int(res_parts[1]) if len(res_parts) == 2 else 1920
        ...
        detector_model_path = get_detector_path()
        base_opts = core.base_options.BaseOptions(model_asset_path=detector_model_path)
        ...
        if logo_path and os.path.exists(logo_path):
            try:
                ...
                nlw = 180
                nlh = int(lh * (nlw / lw))
                img_logo = img_logo.resize((nlw, nlh), PILImage.Resampling.LANCZOS)
        ...
        if not thumb_img_input_path.exists():
            ...
            for _ in range(int(fps * 2)):
                ...
                if res.detections:
                    ...
                    if score > best_score:
                        best_score = score
                        ch_val = int(frame_h/zoom)
                        cw_val = int(frame_h*9/16/zoom)
                        cw_val, ch_val = min(cw_val, frame_w), min(ch_val, frame_h)
                        y_off = y_offset_factor if y_offset_factor != 0.35 else 0.4
                        cx_val, cy_val = f.shape[1]//2, f.shape[0]//2
                        if res.detections:
                            b = res.detections[0].bounding_box
                            cx_val, cy_val = b.origin_x + b.width//2, b.origin_y + b.height//2
                        x1 = max(0, min(int(cx_val)-cw_val//2, frame_w-cw_val))
                        y1_val = max(0, min(int(cy_val)-int(ch_val*y_off), frame_h-ch_val))
                        crop = apply_sharpen(cv2.resize(f[y1_val:y1_val+ch_val, x1:x1+cw_val].copy(), (target_w, target_h), interpolation=cv2.INTER_CUBIC), strength=0.25)
                        ...
                        pil_c = PILImage.fromarray(cv2.cvtColor(eff, cv2.COLOR_BGR2RGB))
                        d_c = ImageDraw.Draw(pil_c)
                        if judul_opini and pil_font_opini:
                            ...
                            overlay = PILImage.new('RGBA', pil_c.size, (0,0,0,0))
                            d_ov = ImageDraw.Draw(overlay)
                            d_ov.rectangle([0, 20, target_w, 20 + total_h], fill=(0,0,0,255))
                            pil_c = PILImage.alpha_composite(pil_c.convert('RGBA'), overlay).convert('RGB')
                            d_c = ImageDraw.Draw(pil_c)
        ...
        for frame_num in range(total_frames):
            ...
            if detect_this_frame:
                ...
                det_w_val = int(frame_w * det_scale)
                det_h_val = int(frame_h * det_scale)
                det_frame = cv2.resize(frame, (det_w_val, det_h_val), interpolation=cv2.INTER_AREA)
                ...
                if results.detections:
                    for det in results.detections:
                        ...
                        if score < 0.5:
                            continue
                        if score > max_score_in_frame:
                            max_score_in_frame = score
                        b = det.bounding_box
            ...
            if zoom_punch_state["active"]:
                if cur_time < zoom_punch_state["end_time"]:
                    progress_val = (cur_time - (zoom_punch_state["end_time"] - ZOOM_PUNCH_DURATION)) / ZOOM_PUNCH_DURATION
                    if progress_val < 0.5:
                        punch = 1.0 + (ZOOM_PUNCH_FACTOR - 1.0) * (progress_val * 2)
                    else:
                        punch = ZOOM_PUNCH_FACTOR - (ZOOM_PUNCH_FACTOR - 1.0) * ((progress_val - 0.5) * 2)
            ...
            img_pil = PILImage.fromarray(cv2.cvtColor(frame_effects, cv2.COLOR_BGR2RGB))
            ...
            for row in range(grad_start, target_h):
                progress_val = (row - grad_start) / (target_h - grad_start)
                alpha = int(200 * progress_val ** 2.5)
                if alpha > 0:
            ...
            if ffmpeg_proc.poll() is not None:
                if not ffmpeg_errored:
                    ffmpeg_errored = True
                    stderr_lines = []
                    while not ff_err_q.empty():
                        stderr_lines.append(ff_err_q.get_nowait().decode("utf-8", errors="replace"))
