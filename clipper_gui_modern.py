...
class SettingsDialog(ctk.CTkToplevel):
    def __init__(self, parent, config, on_save):
        ...
        ctk.CTkLabel(self.g_f, text="Gemini API Key:", text_color="#8D99AE").grid(row=0, column=0, padx=10, pady=5, sticky="w"); self.gk_var = ctk.StringVar(value=config.get("gemini_api_key", "")); ctk.CTkEntry(self.g_f, textvariable=self.gk_var, width=300, show="*", corner_radius=8).grid(row=0, column=1, padx=10, pady=5, sticky="ew")
        ctk.CTkLabel(self.g_f, text="Gemini Model:", text_color="#8D99AE").grid(row=1, column=0, padx=10, pady=5, sticky="w"); self.gm_var = ctk.StringVar(value=config.get("gemini_model", "gemini-2.0-flash")); ctk.CTkComboBox(self.g_f, values=["gemini-2.0-flash", "gemini-1.5-flash"], variable=self.gm_var, width=300, corner_radius=8).grid(row=1, column=1, padx=10, pady=5, sticky="ew")
        ...
        ctk.CTkLabel(self.o_f, text="OpenRouter Key:", text_color="#8D99AE").grid(row=0, column=0, padx=10, pady=5, sticky="w"); self.ok_var = ctk.StringVar(value=config.get("openrouter_api_key", "")); ctk.CTkEntry(self.o_f, textvariable=self.ok_var, width=300, show="*", corner_radius=8).grid(row=0, column=1, padx=10, pady=5, sticky="ew")
        ctk.CTkLabel(self.o_f, text="Model ID:", text_color="#8D99AE").grid(row=1, column=0, padx=10, pady=5, sticky="w"); self.om_var = ctk.StringVar(value=config.get("openrouter_model", "nvidia/nemotron-3-super-120b-a12b:free")); ctk.CTkEntry(self.o_f, textvariable=self.om_var, width=300, corner_radius=8).grid(row=1, column=1, padx=10, pady=5, sticky="ew")
        ...
        ctk.CTkLabel(self.gr_f, text="Groq API Key:", text_color="#8D99AE").grid(row=0, column=0, padx=10, pady=5, sticky="w"); self.grk_var = ctk.StringVar(value=config.get("groq_api_key", "")); ctk.CTkEntry(self.gr_f, textvariable=self.grk_var, width=300, show="*", corner_radius=8).grid(row=0, column=1, padx=10, pady=5, sticky="ew")
        ctk.CTkLabel(self.gr_f, text="Model ID:", text_color="#8D99AE").grid(row=1, column=0, padx=10, pady=5, sticky="w"); self.grm_var = ctk.StringVar(value=config.get("groq_model", "llama-3.3-70b-versatile")); ctk.CTkEntry(self.gr_f, textvariable=self.grm_var, width=300, corner_radius=8).grid(row=1, column=1, padx=10, pady=5, sticky="ew"); r += 1
        ctk.CTkLabel(self, text="Whisper Provider:", font=("Arial", 13, "bold"), text_color="#2B2D42").grid(row=r, column=0, padx=20, pady=10, sticky="w")
        ...
        ctk.CTkLabel(self, text="Whisper Model (API):", font=("Arial", 13), text_color="#8D99AE").grid(row=r, column=0, padx=20, pady=5, sticky="w")
        ...
        ctk.CTkLabel(self, text="Template:", font=("Arial", 14, "bold"), text_color="#2B2D42").grid(row=r, column=0, padx=20, pady=10, sticky="w")
        ...
        ctk.CTkLabel(self, text="Export Resolusi:", font=("Arial", 13), text_color="#2B2D42").grid(row=r, column=0, padx=20, pady=10, sticky="w")
        ...
        ctk.CTkLabel(self, text="Silence Threshold (detik):", font=("Arial", 13), text_color="#2B2D42").grid(row=r, column=0, padx=20, pady=10, sticky="w")
        ...
        ctk.CTkLabel(self, textvariable=self.st_var, text_color="#8D99AE", width=40).grid(row=r, column=1, padx=(220,0), pady=10, sticky="w"); r += 1
        self.ec_var = ctk.BooleanVar(value=config.get("end_card", True))
        ctk.CTkCheckBox(self, text="End Card (CTA di akhir video)", variable=self.ec_var, fg_color="#2B2D42").grid(row=r, column=0, columnspan=2, padx=20, pady=5, sticky="w"); r += 1
        ctk.CTkLabel(self, text="End Card Text:", font=("Arial", 13), text_color="#2B2D42").grid(row=r, column=0, padx=20, pady=5, sticky="w")
        ...
        ctk.CTkButton(self, text="👁️ Preview Subtitle Style", command=self.preview_subtitle, fg_color="#2B2D42", hover_color="#1A1C2E", corner_radius=8).grid(row=r, column=0, columnspan=2, padx=20, pady=10); r += 1

        ctk.CTkButton(self, text="🔍 Test API Key", command=self.test_api, fg_color="#F0DDD2", text_color="#2B2D42", hover_color="#E8CFC0", corner_radius=10, height=36).grid(row=r, column=0, columnspan=2, padx=20, pady=8, sticky="ew"); r+=1
        b_f = ctk.CTkFrame(self, fg_color="transparent"); b_f.grid(row=r, column=0, columnspan=2, pady=20); ctk.CTkButton(b_f, text="💾 Save", command=self.save, fg_color="#2B2D42", hover_color="#1A1C2E", corner_radius=8, width=100).pack(side="left", padx=20); ctk.CTkButton(b_f, text="Batal", fg_color="#8D99AE", hover_color="#6B7A90", corner_radius=8, width=100, command=self.destroy).pack(side="left", padx=20); self.toggle_ai(self.p_var.get())
    ...
    def test_api(self):
        ...
        def _t():
            try:
                cfg = {"ai_provider": self.p_var.get(), "gemini_api_key": self.gk_var.get().strip(), "gemini_model": self.gm_var.get(), "openrouter_api_key": self.ok_var.get().strip(), "openrouter_model": self.om_var.get().strip(), "groq_api_key": self.grk_var.get().strip(), "groq_model": self.grm_var.get().strip()}
                res = safe_generate_content(cfg, "Balas dengan kata OK saja.", None)
                messagebox.showinfo("Test API", f"✅ Berhasil! Response: {res[:200]}")
            except Exception as e:
...
class VideoItem(ctk.CTkFrame):
    def __init__(self, master, index, remove_cb, log_func, config, get_link):
        super().__init__(master, fg_color="#FFFFFF", corner_radius=12, border_width=1, border_color="#F0DDD2")
        self.index, self.remove_cb, self.log_func, self.config, self.get_link = index, remove_cb, log_func, config, get_link; self.is_json = False; self.sel_var = ctk.BooleanVar(value=True); ctk.CTkCheckBox(self, text="", variable=self.sel_var, width=20, fg_color="#2B2D42", hover_color="#1A1C2E").grid(row=0, column=0, rowspan=2, padx=15, pady=10)
        self.l_f = ctk.CTkFrame(self, fg_color="transparent", width=70); self.l_f.grid(row=0, column=1, rowspan=2, padx=10, pady=10, sticky="nsew"); self.idx_lbl = ctk.CTkLabel(self.l_f, text=f"#{index+1}", font=("Arial", 18, "bold"), text_color="#2B2D42"); self.idx_lbl.pack(); self.st_lbl = ctk.CTkLabel(self.l_f, text="Ready", font=("Arial", 11), text_color="#8D99AE"); self.st_lbl.pack()
        ...
        ctk.CTkLabel(r1, text="Mulai:", text_color="#8D99AE").pack(side="left", padx=3); self.s_var = ctk.StringVar(value="00:00:00"); ctk.CTkEntry(r1, textvariable=self.s_var, width=70, corner_radius=6).pack(side="left", padx=3)
        ctk.CTkLabel(r1, text="Selesai:", text_color="#8D99AE").pack(side="left", padx=3); self.e_var = ctk.StringVar(value="00:00:10"); ctk.CTkEntry(r1, textvariable=self.e_var, width=70, corner_radius=6).pack(side="left", padx=3)
        ...
        ctk.CTkLabel(r1, text="Z:", text_color="#8D99AE").pack(side="left", padx=2); self.z_var = ctk.StringVar(value="1.0"); ctk.CTkEntry(r1, textvariable=self.z_var, width=40, corner_radius=6).pack(side="left", padx=2)
        ctk.CTkLabel(r1, text="Y:", text_color="#8D99AE").pack(side="left", padx=2); self.y_var = ctk.StringVar(value="0.35"); ctk.CTkEntry(r1, textvariable=self.y_var, width=40, corner_radius=6).pack(side="left", padx=2)
        self.sp_var = ctk.BooleanVar(value=False); ctk.CTkCheckBox(r1, text="Split", variable=self.sp_var, width=60, fg_color="#2B2D42").pack(side="left", padx=5)
        r2 = ctk.CTkFrame(self.man_f, fg_color="transparent"); r2.pack(fill="x", pady=(5,0))
        ctk.CTkLabel(r2, text="Judul:", text_color="#8D99AE").pack(side="left", padx=3)
        self.t_var = ctk.StringVar(); ctk.CTkEntry(r2, textvariable=self.t_var, width=160, corner_radius=6).pack(side="left", padx=5)
        ctk.CTkLabel(r2, text="Opini:", text_color="#8D99AE").pack(side="left", padx=3)
        self.thumb_var = ctk.BooleanVar(value=True)
        ctk.CTkCheckBox(r2, text="IMG", variable=self.thumb_var, width=50, fg_color="#2B2D42").pack(side="left", padx=3)
        ...
        self.gem_b = ctk.CTkButton(r2, text="✨ Analisis", width=90, command=self.analyze_with_ai, fg_color="#2B2D42", hover_color="#1A1C2E", corner_radius=8); self.gem_b.pack(side="left", padx=10)
        r3 = ctk.CTkFrame(self.man_f, fg_color="transparent")
        r3.pack(fill="x", pady=(2,0))
        self.br_var = ctk.BooleanVar(value=False)
        self.br_cb = ctk.CTkCheckBox(r3, text="🎨 B-Roll Pexels (otomatis)", variable=self.br_var, fg_color="#2B2D42")
...
class App(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkButton(f_l, text="🔍 Ambil & Analisis", command=self.start_analysis, fg_color="#2B2D42", hover_color="#1A1C2E", corner_radius=8).pack(side="left", padx=(10,15), pady=10)
        self.scr = ctk.CTkScrollableFrame(self, width=1050, height=350, fg_color="#FFF7F0", corner_radius=12); self.scr.grid(row=3, column=0, padx=30, pady=10, sticky="nsew"); self.scr.grid_columnconfigure(0, weight=1)
        f_b = ctk.CTkFrame(self, fg_color="transparent"); f_b.grid(row=4, column=0, pady=15)
        ctk.CTkButton(f_b, text="➕ Tambah Segmen Manual", command=self.add_video_item, width=180, fg_color="#F0DDD2", hover_color="#FFFFFF", corner_radius=8).pack(side="left", padx=10)
        self.p_btn = ctk.CTkButton(f_b, text="▶ PROSES TERPILIH", fg_color="#2B2D42", hover_color="#1A1C2E", width=200, command=self.start_processing, corner_radius=8).pack(side="left", padx=30)
        self.p_bar = ctk.CTkProgressBar(self, width=1050, corner_radius=5, fg_color="#FFFFFF", progress_color="#2B2D42"); self.p_bar.grid(row=5, column=0, pady=5, padx=30); self.p_bar.set(0)
        self.log_t = ctk.CTkTextbox(self, width=1050, height=180, fg_color="#FFF7F0", text_color="#2B2D42", corner_radius=12, border_width=1, border_color="#F0DDD2"); self.log_t.grid(row=6, column=0, padx=30, pady=(5,20), sticky="nsew")
        sys.stdout = self
    ...
    def run_analysis(self, link):
        try:
            ...
            try:
                subprocess.run(base_subs, capture_output=True, timeout=60)
