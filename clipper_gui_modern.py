...
class App(ctk.CTk):
    ...
    def run_analysis(self, link):
        try:
            ...
            cmd_info = f'{y_p} {c_o} --user-agent "{UA}" --extractor-args "youtube:player_client=tv,web_creator,mediaconnect" --skip-download --write-info-json -o "{TEMP_DIR}/{sid}_full" "{link}"'
            ...
            cmd_subs = f'{y_p} {c_o} --user-agent "{UA}" --extractor-args "youtube:player_client=tv,web_creator,mediaconnect" --skip-download --write-auto-subs --sub-langs "id,en" --convert-subs srt -o "{TEMP_DIR}/{sid}_full" "{link}"'
            try:
