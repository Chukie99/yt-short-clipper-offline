...
def run_analysis(link, ai_provider, gemini_key, groq_key, openrouter_key,
    ...
    try:
        ...
        cmd_info = f'{y_p} {cookies_opts} --user-agent "{UA}" --extractor-args "youtube:player_client=tv,web_creator,mediaconnect" --skip-download --write-info-json -o "{TEMP_DIR}/{sid}_full" "{link}"'
        import subprocess
        ...
        # Try subtitles
        cmd_subs = f'{y_p} {cookies_opts} --user-agent "{UA}" --extractor-args "youtube:player_client=tv,web_creator,mediaconnect" --skip-download --write-auto-subs --sub-langs "id,en" --convert-subs srt -o "{TEMP_DIR}/{sid}_full" "{link}"'
        try:
...
def build_ui():
    with gr.Blocks(
        ...
        **AI-Powered YouTube Shorts Generator** — Jalankan dari Google Colab atau browser mana saja.
        ...
        gr.Markdown("""
        ---
        **YT Short Clipper v1.1.0** — [GitHub](https://github.com/Chukie99/yt-short-clipper-offline) | Made with ❤️
        """)
